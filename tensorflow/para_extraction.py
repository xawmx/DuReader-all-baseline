...
import json
import copy
from utils.preprocess import metric_max_over_ground_truths,f1_score
...
def paragraph_selection(sample, mode):
    ...
    if 'answer_docs' in sample and len(sample['answer_docs']) > 0:
        ...
        if doc_id >= len(sample['documents']):
            ...
            # will be filtered by dataset-lu.py
            return
...
if __name__ == "__main__":
    ...
    # mode = sys.argv[1]
    dic=[]
    with open('../data/preprocessed/devset/all_devset.json', encoding='utf-8') as fin:
        for line in fin:
            line=line.strip()
            if line == "":
                continue
    ...
    for dics in dic:
        # print(dics)
        jsonData = json.dumps(dics,ensure_ascii=False)
        with open('../data/preprocessed/devset/new_dev.json', 'a+', encoding='utf-8') as fin:
              fin.write(jsonData)
              fin.write('\n')
    print("End")
