...
EMBEDDING_DIM = 128
# ['Depends'] ['No'] ['Yes']' 'No_Opinion'

emotion2id = {'Yes': 1, 'No': 0, 'Depends': 2}
    ...
    with open(path, encoding='utf-8') as fin:
        for line in fin:
            ...
            if (sample['question_type'] == 'YES_NO'):
                if mode=='dev':
                    print(print(sample))
                    if sample['yesno_answers'] == 'Depends':
                        print(sample)
                text = sample['answers'][0]
                if mode != 'test':
                    l = emotion2id[sample['yesno_answers'][0]]
                    label.append(l)
                text = re.sub(r' ', '', text)
...
def get_model(embedding, class_num=3):
    ...
    print("inputs_sentence:", inputs_sentence.shape)

    sentence = SpatialDropout1D(0.2)(embedding(inputs_sentence))
    # context, _ = rnn('bi-lstm', sentence, 600, 128, layer_num=1)
...
test_path = '../data/results/test.predicted.json'

x_train, y_train = processing_data(train_path,mode='train')
x_dev, y_dev = processing_data(dev_path,mode='dev')
x_test, _ = processing_data(test_path, mode='test')
...
w2v_model = KeyedVectors.load_word2vec_format('word2vec.txt')
not_words = []
...
np.savetxt('test_lstm_attention.txt', y_test)
